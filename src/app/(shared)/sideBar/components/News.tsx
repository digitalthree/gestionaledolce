...
const News: React.FC<NewsProps> = ({editabile}) => {
    ...
    const news: { id: number, data: string, titolo: string, testo: string }[] = [
        {
            id: 0,
            data: "2023-07-18",
            titolo: "Titolo News 1",
            testo: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book."
        },
        {
            id: 1,
            data: "2023-07-19",
            titolo: "Titolo News 2",
            testo: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book."
        },
        {
            id: 2,
            data: "2023-07-20",
            titolo: "Titolo News 3",
            testo: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book."
        },
        {
            id: 3,
            data: "2023-07-20",
            titolo: "Titolo News 4",
            testo: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book."
        },
    ]

    const [inputNews, setInputNews] = useState<{ id: number, data: string, titolo: string, testo: string }[]>(news)
    const [newNews, setNewNews] = useState<{ id: number, data: string, titolo: string, testo: string }>({
        id: inputNews.length,
    ...
    return (
        <div className="flex flex-col">
            <div className="flex justify-end">
                <h3 className="text-white text-xl font-semibold mb-5">Ultime News</h3>
            </div>
                            ...
                            <li key={n.id} className="mb-3">
                                <div className="flex flex-row items-center justify-between">
                                    <label htmlFor={`my_modal_${n.id}`}
                                           className="px-4 py-1 bg-[#ced4f0] rounded-3xl text-[13px] text-white uppercase font-semibold hover:cursor-pointer hover:opacity-70">{dat[2]}:{dat[1]} | {n.titolo.substring(0, 12)}...</label>
                                    {editabile &&
                                        <>
                                            <label htmlFor={`my_modal_${n.id}`} className="hover:cursor-pointer hover:text-[#2866CC] text-white">
                                                <RiSettings5Fill size={22}/>
                                            </label>

                                            <BsTrash2Fill size={22} className="hover:cursor-pointer hover:text-[#2866CC] hover:opacity-70 text-white" onClick={() => {
                                    ...
                                    <div className="modal-box max-w-2xl h-[600px] border-[5px] border-[#B5C5E7]">
                                        ...
                                        {editabile ?
                                            <div
                                                ...
                                                <label htmlFor={`my_modal_${n.id}`}
                                                       className="btn bg-[#B5C5E7] hover:bg-[#B5C5E7] hover:opacity-70 text-white w-1/2">Aggiorna
                                                    News</label>
                                            </div> :
                                            <div className="modal-action absolute right-0 left-0 p-3 bottom-5 w-full">
                                                <label htmlFor={`my_modal_${n.id}`}
                ...
                <>
                    ...
                    <div className="modal">
                        <div className="modal-box max-w-2xl h-[600px] border-[5px] border-[#B5C5E7]">
                            ...
                            />
                            <textarea className="hover:cursor-pointer text-[#B5C5E7] w-full h-[60%] p-2 border-2 border-[#B5C5E7] rounded"
                                      id={"textAreaNews"}
                                      value={newNews.testo}
                            ...
                            >Inserisci testo</textarea>
                            <div className="modal-action absolute right-0 left-0 p-3 bottom-5 flex justify-around w-full">
