...
const News: React.FC<NewsProps> = ({editabile}) => {
    ...
    const [inputNews, setInputNews] = useState<{ id: number, data: string, titolo: string, testo: string }[]>(news.reverse())
                                    ...
                                    <li key={n.id} className="mb-3">
                                        <div className="flex flex-row items-center justify-between">
                                            <label htmlFor={`my_modal_${n.id}`}
                                                   className="px-4 py-2 bg-white rounded-3xl text-[14px] text-[#2866CC] uppercase font-semibold hover:cursor-pointer hover:opacity-70">{dat[2]}:{dat[1]} | {n.titolo.substring(0, 30)}...</label>
                                            {editabile &&
                                                <>
